...
library(ggplot2)
library(sjPlot)
d <- read.csv("wide_form_vowel_means_by_speaker (1).csv")
w2 <- read.csv("2nd_wave_coding (2).csv")
q <- read.csv("qualtrics_for_stats (1).csv")
...
```{r}
#repeat on male group
ggplot(m, aes(x= is_Greek, y= Diag_35.TRAP, color = Ethnicity )) +
  geom_point(position = "jitter") 
...
for (meas in measurements) {
  run_speaker_level_stats(m, meas)
}
